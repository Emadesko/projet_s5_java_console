...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Compte compte = null;
        Role role = null;
        boolean debut= false,arret=true;
        do {
            
            if (compteView.getCompteService().getAll().isEmpty()) {
                role = Role.Admin;
                debut = true;
            }else{
                compte = compteView.connexion();
                if (compte != null) {
                    role = compte.getRole();
                    debut = true;
                }else{
                ...
                switch (role) {
                    case Admin:
                        do {
                            ...
                            switch (choix) {
                                case 1:
                                    compteView.saisie(compteView.getCompteService().getAll().isEmpty()?Role.Admin:null,clientView);
                                    break;
                    ...
                    case Boutiquier:
                        ...
                        break;
                    case Client:
                        Client client = clientView.getClientService().getClientByCompte(compte);
                        do {
